...
public class MyDatabase {
	...
	public static void main(String[] args) {
		try {
			...
			RandomAccessFile csv = new RandomAccessFile("PHARMA_TRIALS_1000B.csv", "rw");
			...
			// insert a new record into db file
				...
				System.out
						.println("Please type the insert query starting from 1001, i.e. INSERT INTO data.db VALUES (1001,AAA,aa-111,1,11,111,11.1,TRUE,FALSE,TRUE,FALSE)");
				Scanner scanner2 = new Scanner(System.in);
			...
			// delete an item from db file
				...
				System.out.println("Please type the delete query, i.e. DELETE FROM data.db WHERE id = 1001");
				Scanner scanner2 = new Scanner(System.in);
				String deleteCommand = scanner2.nextLine();
	...
	public static void parse(RandomAccessFile csv) {

		try {
			String thisLine = csv.readLine();
			thisLine = csv.readLine();
			long pointer = 0;
			RandomAccessFile dbFile = new RandomAccessFile("data.db", "rw");
			// parse till reach the end of the file
			while (thisLine != null) {
				String[] value = thisLine.split(",(?=([^\"]*\"[^\"]*\")*[^\"]*$)");
				if (value[0].trim().equalsIgnoreCase("id"))
					continue;
				pointer = dbFile.getFilePointer();
				// organize the original csv file
				value[1] = value[1].replace("\"", "");
				// match each item to the corresponding data type in the
				// requirement
...
//		System.out.println("flag1");
		...
		for (int i = pos.length - 1; i >= 0; i--) {
			...
			if ((commonByte >> 7) != 1) {
				...
				System.out.println("Some of the data in your query has been deleted and will not be displayed!");
			} else {
	...
	public static void delete(RandomAccessFile dbFile, int i, long offsetNum, String item[]) throws IOException {

		String offset[] = item[1].split(",");
		for (i = offset.length - 1; i >= 0; i--) {
			offsetNum = Long.parseLong(offset[i]);
			dbFile.seek(offsetNum);
